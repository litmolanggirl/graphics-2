...
Vector3f& Vector3f::Normalize()
{
    ...
    //нормируем вектор (делаем вектор единичной длины)
    x /= Length;
    y /= Length;
    z /= Length;
...
public:
    Pipeline()
    {
        ...
        m_worldPos = Vector3f(1.0f, 1.0f, 1.0f);
...
//инициализируем три отдельные матрицы для преобразования с текущимим параметрамии
...
static void RenderSceneCB()
{
    ...
    p.WorldPos(0.0f, 0.0f, 3.0f);  //позиция объекта
    Vector3f CameraPos(0.0f, sinf(Speed), -3.0f);  //позиция камеры
...
{
    Vector3f Vertices[4];   //увеличение массива, чтобы он мог содержать 4 вершины
    Vertices[0] = Vector3f(-1.0f, -1.0f, 0.5773f);
    Vertices[1] = Vector3f(0.0f, -1.0f, -1.15475);
    Vertices[2] = Vector3f(1.0f, -1.0f, 0.5773f);
    Vertices[3] = Vector3f(0.0f, 1.0f, 0.0f);

    glGenBuffers(1, &VBO);  //создание буфера вершин (первый - количество объектов, которое хотим создать, а второй — адрес массива для хранения дескрипторов)
...
{
    //буфер индексов заполняется с помощью массива индексов
    ...
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, IBO);   //привязка объекта именованного буфера
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(Indices), Indices, GL_STATIC_DRAW);   //заполнение объекта данными
}
...
int main(int argc, char** argv)
{
    ...
    glutCreateWindow("Tutorial 13");    //заголовок окна
